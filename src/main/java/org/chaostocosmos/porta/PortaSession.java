...
public class PortaSession implements Runnable {
		...
		while (!this.isDone) {
			try {
				...
				if (sessionMapping.getRemoteHosts().stream().anyMatch(h -> sessionMapping.isForbiddenHost(h))) {
					String err = "Forbidden remote host request detected. Forbidden List: "
							+ sessionMapping.getRemoteHosts().toString();
					logger.info(err);
					closeSocket(client, err);
				} else {
					if (sessionMapping.getAllowedHosts().size() == 0 || sessionMapping.getAllowedHosts().stream()
							.anyMatch(h -> h.equals(client.getLocalAddress().getHostAddress()))) {
						logger.info("[" + sessionName + "][SESSION MODE: " + this.sessionMapping.getSessionModeEnum()
								+ "] Client channel connected. " + client.getRemoteSocketAddress().toString());
						SessionTask task = new SessionTask(sessionName, sessionMapping, client);
						logger.info("[" + sessionName + "] new client is connected. Clinet: "
								+ client.getRemoteSocketAddress().toString() + " ------------------- connected.");
	...
	public class SessionTask extends Thread {
		 ...
		 * @throws IOException
		 */
		private void startInteraction(String sessionName, Socket client, PortaSocket remote) throws Exception {
			try {
				InteractiveChannel sendChannel = new InteractiveChannel(sessionName + "-Send", client.getInputStream(),
						remote.getOutputStream(), transactionStartMillis);
				InteractiveChannel receiveChannel = new InteractiveChannel(sessionName + "-Receive",
						remote.getInputStream(), client.getOutputStream(), transactionStartMillis);
				// sendChannel.start();
		...
		public void run() {
			...
			try {
				...
				case STAND_ALONE:
					...
					PortaSocket proxySocket = createProxySocket(sessionMapping, 0);
					...
					for (int i = 0; i < retry; i++) {
						try {
							...
							logger.info("[" + sessionName + "][SESSION MODE: "
									+ this.sessionMapping.getSessionModeEnum() + "] Remote channel connected. "
									+ proxySocket.getRemoteAddress().toString());
				...
				case HIGI_AVAILABLE_FAIL_OVER:
					...
					if (masterSessionFailed && slaveSessionFailed) {
						failCount++;
						logger.info("[" + sessionName + "][SESSION MODE: " + this.sessionMapping.getSessionModeEnum()
								+ "] Master/Slave channel disable now. Please reboot or rivival using Admin CLI(Currently not support).");
					}
					if (!masterSessionFailed) {
						PortaSocket master = createProxySocket(sessionMapping, 0);
						try {
							master.connect();
							startInteraction(sessionName, client, master);
							logger.info(
									"[" + sessionName + "][SESSION MODE: " + this.sessionMapping.getSessionModeEnum()
											+ "] Master channel connected. " + master.getRemoteAddress().toString());
							masterSessionFailed = false;
							totalSuccessCount++;
						} catch (Exception e) {
							if (master != null)
							...
							logger.info(
									"[" + sessionName + "][SESSION MODE: " + this.sessionMapping.getSessionModeEnum()
											+ "] Master channel failed. Slave channel be used at next connection.");
							logger.throwable(e);
						}
					}
					if (masterSessionFailed && !slaveSessionFailed) {
						PortaSocket slave = createProxySocket(sessionMapping, 1);
						try {
							slave.connect();
							startInteraction(sessionName, client, slave);
							logger.info(
									"[" + sessionName + "][SESSION MODE: " + this.sessionMapping.getSessionModeEnum()
											+ "] Slave channel connected. " + slave.getRemoteAddress().toString());
							slaveSessionFailed = false;
							totalSuccessCount++;
						} catch (Exception e) {
							if (slave != null)
							...
							logger.info(
									"[" + sessionName + "][SESSION MODE: " + this.sessionMapping.getSessionModeEnum()
											+ "] Slave channel failed. Master/Slave failed.");
							logger.throwable(e);
						}
					}
					if (masterSessionFailed && slaveSessionFailed) {
				...
				case LOAD_BALANCE_ROUND_ROBIN:
					while (loadBalancedStatus.values().stream().anyMatch(b -> b == true)) {
						...
						PortaSocket channel = createProxySocket(sessionMapping, channelIndex);
						try {
							channel.connect();
							...
							logger.info(
									"[" + sessionName + "][SESSION MODE: " + this.sessionMapping.getSessionModeEnum()
											+ "] Load-Balanced Round-Robin channel connected. Channel index: "
											+ channelIndex + "  Host: " + channel.getRemoteAddress().toString());
							break;
						} catch (Exception e) {
							logger.info("[" + sessionName + "][SESSION MODE: " + sessionMapping.getSessionModeEnum()
									+ "] Load-Balance Round-Robin Channel failed. Round-Robin channel index: "
									+ channelIndex + "  Host: " + channel.getRemoteAddress().toString());
							if (channel != null)
								channel.close();
							...
							if (!loadBalancedStatus.values().stream().anyMatch(b -> b == true)) {
								logger.info("[" + sessionName + "][SESSION MODE: " + sessionMapping.getSessionModeEnum()
										+ "] All of Load-Balanced remote channel failed. Please check remote hosts healthy. eg. Network, Server status...");
								if (client != null)
									client.close();
								if (channel != null)
									channel.close();
				...
				case LOAD_BALANCE_SEPARATE_RATIO:
					...
					while (loadBalancedStatus.values().stream().anyMatch(b -> b == true)) {
						...
						PortaSocket channel = createProxySocket(sessionMapping, randomIndex);
						try {
							channel.connect();
							startInteraction(sessionName, client, channel);
							totalSuccessCount++;
							logger.info("[" + sessionName + "][SESSION MODE: "
									+ this.sessionMapping.getSessionModeEnum()
									+ "] Load-Balance Separate Ratio Distribution Channel connected. Channel index: "
									+ randomIndex + "  Host: "
									+ sessionMapping.getRemoteHosts().get(randomIndex).toString());
							break;
						} catch (Exception e) {
							logger.info("[" + sessionName + "][SESSION MODE: "
									+ this.sessionMapping.getSessionModeEnum()
									+ "] Load-Balance Separate Ratio Distribution Channel failed. Channel index: "
									+ randomIndex + "  Host: "
									+ sessionMapping.getRemoteHosts().get(randomIndex).toString());
							if (channel != null)
								channel.close();
							...
							if (!loadBalancedStatus.values().stream().anyMatch(b -> b == true)) {
								logger.info("[" + sessionName + "][SESSION MODE: "
										+ this.sessionMapping.getSessionModeEnum()
										+ "] All of Load-Balance Separate Ratio Distribution remote channel failed. Please check remote host healthy. eg. Network, Server status...");
								if (client != null)
									client.close();
								if (channel != null)
									channel.close();
				...
				logger.info("[" + sessionName + "][SESSION MODE: " + this.sessionMapping.getSessionModeEnum()
						+ "] Statistics:  Success = " + totalSuccessCount + "  Fail = " + totalFailCount + "");
			} catch (Exception e) {
				logger.throwable(e);
			}
		}
	...
	public class InteractiveChannel extends Thread {
		 ...
		 * @throws IOException
		 */
		public InteractiveChannel(String channelName, InputStream is, OutputStream os, long startMillis)
				throws IOException {
		...
		public void close() throws IOException {
			this.is.close();
			this.os.close();
			logger.info("[" + this.channelName + "] Channel Closed.  Channel Elapse Time Millis: "
